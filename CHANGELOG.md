...
- Silenced a `MaxListenersExceededWarning` ("11 abort listeners added to
  [AbortSignal]") printed on Node 18 when more than ten checkers ran
  concurrently. The orchestrators now lift the per-signal listener cap; the
  listeners were always legitimate (no leak), this just removes the noise.
